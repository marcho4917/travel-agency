...
describe('Component OrderOption', () => {
  it('should render without crashing', () => {
    const component = shallow(<OrderOption type='optionType' name='optionName' />);
  ...
  it('check that the props name content is displayed in the title', () => {
    const expectedContent = 'someTitle';
    const component = shallow(<OrderOption name={expectedContent} type='checkboxes' />);
...
for(let type in optionTypes){
  describe(`Component OrderOption with type=${type}`, () => {
    ...
    switch (type) {
      ...
      case 'checkboxes': {
        it('renders div with class checkboxes', () => {
          const excpectedClass = renderedSubcomponent.find('.checkboxes');
          expect(excpectedClass.length).toBe(1);
        });
      ...
      case 'date': {
        it('should contains DatePicker', () => {
          const expectedContent = renderedSubcomponent.find(DatePicker);
          expect(expectedContent.length).toBe(1);
        });

        it('should run setOrderOption function on change', () => {
          const expectedContent = renderedSubcomponent.find(DatePicker);
          expectedContent.simulate('change', testValue);
          expect(mockSetOrderOption).toBeCalledTimes(1);
          expect(mockSetOrderOption).toBeCalledWith({ [mockProps.id]: testValue });
        });
        break;
